...
min_games_per_wind_condition <- 15

# First: classify each game by wind condition
df_parsed <- df_parsed %>%
  mutate(
    Wind_Condition = case_when(
      Wind_Avg < 10 ~ "Light",
      Wind_Avg >= 10 & Wind_Avg <= 19 ~ "Moderate",
      Wind_Avg > 19 ~ "Strong",
      TRUE ~ NA_character_
    ),
    Wind_Condition = factor(Wind_Condition, levels = c("Light", "Moderate", "Strong"))
  )
...
# Now summarize by Player and wind condition
player_wind_summary_all <- player_games %>%
  filter(!is.na(Wind_Condition)) %>%
  group_by(Player, Wind_Condition) %>%
  summarise(
    Games = n(),
    Wins = sum(Won, na.rm = TRUE),
...
players_with_all_conditions <- player_wind_summary_all %>%
  group_by(Player) %>%
  filter(
    all(c("Light", "Moderate", "Strong") %in% Wind_Condition),
    all(Games >= min_games_per_wind_condition)
  ) %>%
  ungroup() %>%
  distinct(Player)

player_wind_summary <- player_wind_summary_all %>%
  semi_join(players_with_all_conditions, by = "Player") %>%
  group_by(Player) %>%
  mutate(Total_Games = sum(Games)) %>%
  ungroup() %>%
  select(Player, Total_Games, Wind_Condition, Games, Wins, Win_Pct)

player_order <- player_wind_summary %>%
  filter(Wind_Condition == "Strong") %>%
  arrange(Win_Pct) %>%
  pull(Player)

player_wind_summary <- player_wind_summary %>%
  mutate(Player = factor(Player, levels = player_order)) %>%
  arrange(Player, Wind_Condition)
 
print(n = 100, player_wind_summary[-c(16,17,18),])

ggplot(player_wind_summary, aes(x = Wind_Condition, y = Win_Pct, group = Player)) +
  geom_line(color = "gray60", linewidth = 0.8) +
  geom_point(aes(color = Wind_Condition), size = 3) +
  facet_wrap(~ Player) +
  scale_color_manual(values = c("Light" = "steelblue", "Moderate" = "goldenrod", "Strong" = "tomato")) +
  labs(
    title = "Win Percentage by Wind Condition",
    subtitle = paste0(
      "Light: Wind_Avg < 10 mph; Moderate: 10-19 mph; Strong: > 19 mph. ",
      "Players shown have at least ", min_games_per_wind_condition, " games in each condition."
    ),
    x = "Wind condition",
    y = "Win percentage",
    color = "Wind condition"
  ) +
  theme_minimal()
...
players_to_plot <- c("Jim Elsner", "Diana Elsner", "JaSun Burdick", "Craig Anderson", "Hank Bass", "Drew Magee", "Jordan Huston", "Scott Schmucker", "Conrad Damon", "John Elsner", "Mike Prost")
